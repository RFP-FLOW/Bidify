...
import { Card, EmptyState, StatGrid, LoadingSkeleton, SearchInput, ManagerPageHeader } from "../../components/ui/Themed";
import { BadgeCheck, ArrowLeft, Trophy, ExternalLink, Check, Clock, Bot } from "lucide-react";
  ...
  return (
    <ManagerLayout>
      ...
      <ManagerPageHeader
        ...
        subtitle="RFPs forwarded to you with AI vendor analysis."
      />
      ...
      <Card className="overflow-hidden animate-fadeIn" delay={250}>
        ...
        {loading && <LoadingSkeleton rows={3} cardHeight="h-24" />}

        {!loading && filtered.length === 0 && (
          <EmptyState icon={BadgeCheck} title={search ? "No results" : "No recommendations yet"}
            subtitle="Forwarded RFPs will appear here." />
        )}
...
function DetailView({ rfp, onBack }) {
  ...
  const handleApprove = async (proposalId) => {
    try {
      const res = await api.patch(`/rfp/proposal/approve/${proposalId}`);
      if (res.data.success) {
        setAiData(prev => ({
          ...prev,
